...
export default function FastOrderModal({ customButton = false }: { customButton?: boolean }) {
  ...
  const [margemMaster, setMargemMaster] = useState<number>(35) // Default 35 as user stated
              ...
              {step === 1 && (
                <form onSubmit={handleParse} className="p-6">
                  ...
                  <textarea 
                    ...
                    placeholder="Ex: pedido de Maria ciclo 6, 1 pefume cod 1235 68 reais , 3 sabonete de 50 e um lapis de olho codigo 12345 50 reais"
                    className="w-full min-h-[160px] p-5 bg-[#f4f7f6] border border-slate-200 focus:border-[#2d7ff9] outline-none rounded-md resize-none text-[#3a4651] placeholder-slate-400 font-medium transition-all text-sm shadow-inner"
                    autoFocus
                  ></textarea>

                  {feedback && (
                    <div className={`mt-4 p-3 rounded text-xs font-bold ${feedback.type === 'success' ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-700'}`}>
                      {feedback.msg}
                    </div>
                  )}
                  ...
                  <div className="mt-6 flex justify-end gap-3">
                    <button 
                      ...
                      className="px-6 py-2.5 bg-[#2d7ff9] hover:bg-[#1e61cc] text-white font-bold text-xs uppercase tracking-wider rounded flex items-center shadow-sm disabled:opacity-50"
                    >
                      {loadingParse ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <ArrowRight className="w-4 h-4 mr-2" />}
                      {loadingParse ? 'Processando Texto...' : 'Extrair Produtos para Grid'}
                    </button>
                  </div>
                </form>
              )}
